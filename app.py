...
def generate_one_ai_problem(text, problem_no):
    prompt = f"""
あなたは JSON 生成専用の関数です。
説明文・コメント・挨拶・前置き・後書きは禁止です。
以下の条件を1つでも満たさない場合、それは「失敗」です。

【タスク】
以下の資料をもとに、薬剤師国家試験形式の五肢択一問題を【1問】作成してください。

【絶対条件（違反厳禁）】
1. 出力は JSON オブジェクト【1つのみ】
2. 配列（[]）で包まない
3. JSON以外の文字を1文字も出力しない
4. キーは必ず以下の5つのみを使用する
   - topic
   - question
   - choices
   - correct
   - explanation
5. choices は必ず A,B,C,D,E の5キーを持つ object
6. correct は A〜E の【1文字のみ】
7. explanation は日本語1〜3文
8. Markdown記法・コードブロックは禁止
